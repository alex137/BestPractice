...
## Story
No dated incident was recorded. What the rule rests on is a claim about
where the analysis actually happens.

**The full matrix invites the reader to do the dominance analysis in their
head, badly.** A cross-product table has more rows than anyone will scan and
most of them are dominated -- some other row is at least as good on every
output the reader ranks by. Shipping it whole does not present the finding;
it delegates the finding to the reader, at the moment they are least
equipped to do it.

The frontier column does that work once, correctly, in code.

**The second argument is about decay, and it is why the answer is a computed
column rather than editorial curation.** A frontier computed by the model is
regenerated by the model. A hand-picked "interesting rows" table silently
rots as the numbers move -- the same drift `docs-track-models` exists to
kill, in row-selection form rather than in the values themselves.

The rule to ask about the optimization outputs before building follows from
the same place: which outputs the reader ranks by is the reader's choice,
and guessing it wrong makes a correct frontier answer the wrong question.
