---
slug:        retirement-deletes-files
title:       "Retiring a mechanism deletes its files, once an audit says nothing points at them"
tier:        on-demand
severity:    default
applies_to:  ["**"]
occasion:    "retiring a mechanism — a workflow, a tool, a vendored tree, a config — that leaves files behind with no remaining job"
gates:       []
index_clause: "delete what the retired mechanism owned; audit first, never on a hunch"
...
---
## Rule
When a mechanism is retired, the files and directories that existed only
...
[tools/precedent_retire_path.py](../tools/precedent_retire_path.py) on the
path and act only on a clean report. Record what went, and why, in
`process/retired_paths.json`.
...
live trigger (pause it and let a cycle pass — a retirement must never be
...
`process/retired_paths.json` is the record: one entry per retirement, each
...
**Pausing is a step on the way to retirement, not an alternative to it.**
A workflow parked on `workflow_dispatch` with its schedule commented out is
mid-retirement, and the only thing separating that from abandonment is
...
mechanism is retired, when the person doing it still knows exactly what it
was for.
...
rather than suspected. Its step 6 said to "retire the old sync workflow
...
## Install
`tools/precedent_check.py`'s `retirement-deletes-files` check reads
`process/retired_paths.json` (absent = `NotApplicable`, the correct state
for a repo that has retired nothing) and fails if any declared path exists
...
every sync, so a retired path arriving back from a mirror is a real path
and not a hypothetical one.

The audit itself is `tools/precedent_retire_path.py`; `--list` prints what
a repo has retired and why. It is in
...
set retires its own tooling as readily as a consuming repo does, and the
