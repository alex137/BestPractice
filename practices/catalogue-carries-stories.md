...
The actual defect was that nothing ever came back for the declared gap, and in the private sources nothing could: `cite-the-incident`'s check lives in [precedent_check.py](../tools/precedent_check.py), which is in neither `ENGINE_FILES` nor `CONSUMER_ENGINE_FILES`, so it has never run inside a practice set at all. The same shape as a status-contract check found unreachable there for the same reason. **A gap declared in a repo that cannot check for it is indistinguishable from one nobody declared.**
