...
| **Promote** | Land this session's own unsaved work on pre-staging first, then move everything waiting on pre-staging into staging: the full check runs on the whole batch, and staging moves only if it passes. |
