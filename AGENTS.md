...
## Resident block (~949 of 2000 token budget, 10 of 108 practices (10 universal))
...
| Why the closing **Next Steps** section of a reply is not optional, and what refuses a turn without one | [tools/precedent_reply_check.py](tools/precedent_reply_check.py) — the requirement is a `reply_check.json` in whichever practice source declares it, never in the engine; `--explain` says what is declared here |
