...
| | |
|---|---|
| Practices in force | 112 |
| **Enforced by a check** | **46 of 112 practices carry a `checked_by`** |
| Advisory only | 66 of 112 practices |
<!--/gen:merge-back-->
...
repo-local practices at all and so no `local/`; the other dependent repo
does, at `local/practices/`, following the recommendation) — the two repos are not
...
(this repo's own `local/`, and the other dependent repo's) already used `"local"`, so
