...
carry a `run_all.sh` driver, which isn't a per-check file and is
**replaced rather than merged**: the consuming repo gets a driver
generated by the engine itself, recorded in `MANIFEST.json` as
`(generated)`, so there is no winner to pick and no collision to flag.
The source copies are not reported as dropped, because they were not
dropped; what *is* reported, per source, is a source driver that does not
declare itself local). It does **not** vendor the engine scripts
([build_views.py](../tools/build_views.py) and friends) — that half
was already solved (clone/copy this repo's `tools/`, per
[INSTALL.md](../INSTALL.md)'s existing vendoring model) and is a
separate concern from the content-merge gap this closes. A real
consumer repo's `AGENTS.md` is also not created by this tool — a real
one already has the generated-block markers from `INSTALL.md`'s
install step; the harness test seeds a minimal one only to
prove the mechanism.
