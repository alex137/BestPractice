...
# the real name behind every practices/*.md link that reads
# "github.com/alex137/BestPractice/blob/precedent-beta-v01/..."), kept
# current every session so merge-runbook step 0b (an export-gate PR opens
# directly against this repo) and the vendor freshness check just below
# both have something to work against, instead of a session having to
# remember to clone it first (practice: session-bootstrap,
# environment-gotchas).
#
# NOT done with `add_repo`, and that was tried directly, not assumed: a
# session's repos are all owned by one account, and add_repo refuses ANY
# cross-owner attach with "cross-tier adds are not supported in v1" -- at
# any requested access level, read included; asking for read-only does not
# create an exception. What DOES work, at any access level, is a repo
# GitHub itself serves anonymously: alex137/BestPractice is public, so this
# session's own git proxy already serves it with no attach step at all
# (confirmed: add_repo on it replies "already available ... nothing was
# attached to the session" and hands back a plain clone URL). A PRIVATE
# cross-tier repo has no such escape hatch -- see
# tools/precedent_access_check.py's own docstring for the incident this
# exact wall caused for a push, from a session rooted elsewhere.
#
# Idempotent and safe against a concurrent turn reusing the same path, per
# that same tool's own warning: never blindly wipe a directory that might
# be mid-clone or might hold something unrelated.
...
if [ -d "$UPSTREAM_CLONE/.git" ]; then
  if ! git -C "$UPSTREAM_CLONE" rev-parse HEAD >/dev/null 2>&1; then
    ...
    if timeout 60 git -C "$UPSTREAM_CLONE" fetch --quiet --depth 1000 origin precedent-beta-v01 2>/dev/null; then
      ...
      # CHECKED-OUT branch -- the exact gotcha the checkout-freshness block
      # above exists to catch on this repo's own branch, and it would have
      # silently gone stale here after the very first session without this.
      # This clone is a pure read-only mirror (nothing is ever committed to
      # it from here, since push to alex137/BestPractice is refused
      # cross-tier regardless of intent), so resetting it hard to match
      # origin/precedent-beta-v01 is always the right move -- but only when
      # the tree is actually clean, in case something inspected it by hand
      # mid-session.
      if [ -z "$(git -C "$UPSTREAM_CLONE" status --porcelain 2>/dev/null || true)" ]; then
        ...
        echo "WARN: $UPSTREAM_CLONE has uncommitted changes -- fetched but NOT reset to origin/precedent-beta-v01, so it may be stale until that tree is clean" >&2
      fi
    else
  ...
  if GIT_LFS_SKIP_SMUDGE=1 timeout 600 git clone --quiet --depth 1 --branch precedent-beta-v01 \
    ...
    echo "WARN: could not clone alex137/BestPractice to $UPSTREAM_CLONE -- merge-runbook step 0b and the vendor freshness check below will need a manual clone this session" >&2
  fi
fi
...
#
# The Precedent-loader layout's own engine freshness check
# (`python3 tools/precedent_vendor_engine.py status <bestpractice-clone>`)
# now HAS a local clone to compare against -- the one kept current just
# above ($UPSTREAM_CLONE) -- but running it stays a deliberate step, same
# as INSTALL.md section 2, "Keep the vendored engine current (consumer
# repos)" always said: the clone existing is not the same decision as
# treating its drift as something to report on every session start.
