...
| 2026-09-05 | [`6e4d48f`](https://github.com/alex137/BestPractice/commit/6e4d48f3ae8e3a2b97bc0de31f940b09812c9f07) — closed the individual-source SessionStart hook's race against its own session's `add_repo` calls (retry + lazy self-heal) | applied as `hooks/individual-source-bootstrap.sh.template`, a new SessionStart hook wired the same way `hooks/session-start.sh` already is | no transfer because codex has no hook mechanism to wire a SessionStart bootstrap into (bootstrap is an environment setup script there, per [README.md](README.md)'s adapter table — same reason as every other hook-mechanism row above) | no transfer, same reason as codex (bootstrap is an instructions-file directive there, not a hook) |
