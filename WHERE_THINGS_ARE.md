...
| The branch sweep's own list — every merged, merged-elsewhere and unmerged branch in this checkout and every declared source, with a clickable delete or compare link on each row | `record/stale-branches.md` — **never hand-edit it**; written by `python3 tools/very_deep_check.py`'s branch sweep and committed like `MAP.md`. Not yet committed here — the first real run must also clear [tools/leak_gate.py](tools/leak_gate.py) on whatever it finds |
