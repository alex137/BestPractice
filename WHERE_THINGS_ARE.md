...
| **Running CI less often in a private repo** — `ci_every_hours` (default 0, every push), the `[skip ci]` commit hook that applies it at no cost, and which repos should keep it at 0 | [spec/CI_CADENCE_PLAN.md](spec/CI_CADENCE_PLAN.md); the value in force here: `python3 tools/precedent_session_check.py` |
